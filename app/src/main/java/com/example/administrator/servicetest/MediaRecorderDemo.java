...
public class MediaRecorderDemo {
    ...
    public static final int MAX_LENGTH = 1000 * 30;// 最大录音时长30秒;
    ...
    public void startRecord() {
        ...
        try {
            ...
            mHandler.postDelayed(mStopRecord, 30*1000);     //30s后自动停止录音，并等待一个新的Service周期开始

            Log.i("ACTION_START", "startTime" + startTime);
        } catch (IllegalStateException e) {
            Log.i(TAG,
                    "Illegal call startAmr(File mRecAudioFile) failed!"
                            + e.getMessage());
            e.printStackTrace();
        } catch (IOException e) {
            Log.i(TAG,
     ...
     */
    private int BASE = 1;
    private int SPACE = 5 * 1000;// 间隔取样时间5s
